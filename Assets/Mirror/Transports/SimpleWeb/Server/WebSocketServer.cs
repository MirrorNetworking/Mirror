...
namespace Mirror.SimpleWeb
{
    public class WebSocketServer
    {
        ...
        public void Listen(int port)
        {
            ...
            Log.Info($"[SimpleWebTransport] Server has started on port {port}");
        ...
        public void Stop()
        {
            ...
            Log.Info("[SimpleWebTransport] Server stopped, Closing all connections...");
            ...
            try
            {
                try
                {
                    while (true)
                    {
                        ...
                        Log.Info($"[SimpleWebTransport] A client connected {conn}");
        ...
        void HandshakeAndReceiveLoop(Connection conn)
        {
            try
            {
                ...
                if (!success)
                {
                    Log.Error($"[SimpleWebTransport] Failed to create SSL Stream {conn}");
                    conn.Dispose();
                    return;
                }
                ...
                if (success)
                {
                    Log.Info($"[SimpleWebTransport] Sent Handshake {conn}");
                }
                else
                {
                    Log.Error($"[SimpleWebTransport] Handshake Failed {conn}");
                    conn.Dispose();
                    return;
                }
                ...
                if (serverStopped)
                {
                    Log.Info("[SimpleWebTransport] Server stops after successful handshake");
                    return;
                }
                ...
                sendThread.Name = $"[SimpleWebTransport] SendLoop {conn.connId}";
                sendThread.Start();
        ...
        public void Send(int id, ArrayBuffer buffer)
        {
            ...
            {
                Log.Warn($"[SimpleWebTransport] Cant send message to {id} because connection was not found in dictionary. Maybe it disconnected.");
            }
        }
        ...
        public bool CloseConnection(int id)
        {
            if (connections.TryGetValue(id, out Connection conn))
            {
                Log.Info($"[SimpleWebTransport] Kicking connection {id}");
                conn.Dispose();
                return true;
            }
            else
            {
                Log.Warn($"[SimpleWebTransport] Failed to kick {id} because id not found");
        ...
        public string GetClientAddress(int id)
        {
            ...
            {
                Log.Error($"[SimpleWebTransport] Cant get address of connection {id} because connection was not found in dictionary");
                return null;
            }
        }
    }
