...
namespace kcp2k.unmanaged
{
    public abstract class KcpPeer
    {
        ...
        static int ReliableMaxMessageSize_Unconstrained(int mtu, uint rcv_wnd) =>
            (mtu - (int)KCPBASIC.OVERHEAD - METADATA_SIZE) * ((int)rcv_wnd - 1) - 1;
        ...
        public static int ReliableMaxMessageSize(int mtu, uint rcv_wnd) =>
            ReliableMaxMessageSize_Unconstrained(mtu, Math.Min(rcv_wnd, KCPBASIC.FRG_LIMIT));
        ...
        protected void Reset(KcpConfig config)
        {
            ...
            // message afterwards.
            kcp.SetMtu((int)((uint)config.Mtu - METADATA_SIZE));
        ...
        void HandleDeadLink()
        {
            ...
            if (kcp.State == -1)
            {
                // pass error to user callback. no need to log it manually.
                // GetType() shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.Timeout, $"{GetType()}: dead_link detected: a message was retransmitted {KCPBASIC.DEADLINK} times without ack. Disconnecting.");
                Disconnect();
            }
        }
