...
namespace Mirror
{
    ...
    public struct NetworkClientConfig
    {
        ...
        public double bufferTime => NetworkServer.config.sendInterval * bufferTimeMultiplier;
        ...
        public float catchupNegativeThreshold; // careful, don't want to run out of snapshots
        ...
        [Range(0, 1)]
        public double catchupSpeed; // 1%

        [Tooltip("Local timeline slowdown in % while slowing down.")]
        [Range(0, 1)]
        public double slowdownSpeed; // 1%

        [Tooltip("Catchup/Slowdown is adjusted over n-second exponential moving average.")]
        public int driftEmaDuration; // shouldn't need to modify this, but expose it anyway
        ...
        public float dynamicAdjustmentTolerance; // 1 is realistically just fine, 2 is very very safe even for 20% jitter. can be half a frame too. (see above comments)

        [Tooltip("Dynamic adjustment is computed over n-second exponential moving average standard deviation.")]
        public int deliveryTimeEmaDuration;   // 1-2s recommended to capture average delivery time
