// NetworkClient as component.
// named "NetClient" as long as static "NetworkClient" still exists.
//
// transition steps:
...
namespace Mirror
{
    ...
    public partial class NetClient : MonoBehaviour
    {
        ...
        // TODO remove after deprecating static NetworkClient class.
        internal static NetClient instance;

        // message handlers by messageId
        internal readonly Dictionary<ushort, NetworkMessageDelegate> handlers =
        ...
        public NetworkIdentity localPlayer { get; internal set; }
        ...
        public bool active => connectState == ConnectState.Connecting ||
        ...
        public bool isHostClient => connection is LocalConnectionToServer;
        ...
        public readonly Dictionary<uint, GameObject> prefabs =
        ...
        internal readonly Dictionary<uint, SpawnHandlerDelegate> spawnHandlers =
            new Dictionary<uint, SpawnHandlerDelegate>();
        internal readonly Dictionary<uint, UnSpawnDelegate> unspawnHandlers =
            new Dictionary<uint, UnSpawnDelegate>();

        // spawning
        // internal for tests
        internal bool isSpawnFinished;

        // Disabled scene objects that can be spawned again, by sceneId.
        internal readonly Dictionary<ulong, NetworkIdentity> spawnableObjects =
        ...
        // TODO GetComponent and show in inspector later
        [HideInInspector] public InterestManagement aoi;
        ...
        {
            // set instance.
            // only allow one for now as long as NetworkClient exists.
            if (instance != null && instance != this)
                throw new Exception($"Detected another NetClient component on {instance.name} while attempting to use NetClient on {name}. This is not supported, please only use one.");
        ...
        // helper function
        bool UnpackAndInvoke(NetworkReader reader, int channelId)
        {
        ...
        // called by Transport
        internal void OnTransportData(ArraySegment<byte> data, int channelId)
        {
            if (connection != null)
            {
        ...
        public void RegisterHandler<T>(Action<T> handler, bool requireAuthentication = true)
            where T : struct, NetworkMessage
        {
        ...
        public bool GetPrefab(uint assetId, out GameObject prefab)
        {
            prefab = null;
            return assetId != 0 &&
        ...
        //       of people use this.
        public void RegisterPrefab(GameObject prefab, uint newAssetId)
        {
            if (prefab == null)
            {
        ...
        public void RegisterPrefab(GameObject prefab, uint newAssetId, SpawnDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
        {
        ...
        //       of people use this.
        // TODO why do we have one with SpawnDelegate and one with SpawnHandlerDelegate?
        public void RegisterPrefab(GameObject prefab, uint newAssetId, SpawnHandlerDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
        {
            if (newAssetId == 0)
            {
        ...
        public void RegisterSpawnHandler(uint assetId, SpawnDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
        {
            // We need this check here because we don't want a null handler in the lambda expression below
            if (spawnHandler == null)
            {
        ...
        // objects - such as when they are constructed dynamically at runtime
        // from configuration data.
        public void RegisterSpawnHandler(uint assetId, SpawnHandlerDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
        {
            if (spawnHandler == null)
            {
                Debug.LogError($"Can not Register null SpawnHandler for {assetId:X4}");
        ...
        public void UnregisterSpawnHandler(uint assetId)
        {
            spawnHandlers.Remove(assetId);
            unspawnHandlers.Remove(assetId);
        }
        ...
        internal bool InvokeUnSpawnHandler(uint assetId, GameObject obj)
        {
        ...
        // spawning ////////////////////////////////////////////////////////////
        internal void ApplySpawnPayload(NetworkIdentity identity, SpawnMessage message)
        {
        ...
        internal bool FindOrSpawnObject(SpawnMessage message, out NetworkIdentity identity)
        {
        ...
        NetworkIdentity SpawnPrefab(SpawnMessage message)
        {
        ...
        readonly List<uint> removeFromSpawned = new List<uint>();
        ...
        // host mode callbacks /////////////////////////////////////////////////
        void OnHostClientObjectDestroy(ObjectDestroyMessage message)
        {
            //Debug.Log($"NetworkClient.OnLocalObjectObjDestroy netId:{message.netId}");
            spawned.Remove(message.netId);
        }

        void OnHostClientObjectHide(ObjectHideMessage message)
        {
        ...
        internal void OnHostClientSpawn(SpawnMessage message)
        {
        ...
        void OnEntityStateMessage(EntityStateMessage message)
        {
        ...
        void OnObjectHide(ObjectHideMessage message) => DestroyObject(message.netId);

        internal void OnObjectDestroy(ObjectDestroyMessage message) => DestroyObject(message.netId);

        internal void OnSpawn(SpawnMessage message)
        {
        ...
        internal void OnChangeOwner(ChangeOwnerMessage message)
        {
        ...
        internal void ChangeOwner(NetworkIdentity identity, ChangeOwnerMessage message)
        {
            ...
            // identity is now local player. set our helper field to it.
            if (identity.isLocalPlayer)
            {
                localPlayer = identity;
            }
            // identity's isLocalPlayer was set to false.
            // clear our localPlayer IF (and only IF) it was that one before.
        ...
        internal void CheckForLocalPlayer(NetworkIdentity identity)
        {
