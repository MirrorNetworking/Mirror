    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        public Guid assetId
        {
            ...
            internal set
            {
                ...
                else MirrorLog.LogWarning($"SetDynamicAssetId object {this.name} already has an assetId {m_AssetId}, new asset id {newAssetIdString}");
            }
        }
        ...
        internal void SetClientOwner(NetworkConnection conn)
        {
            ...
            {
                MirrorLog.LogError($"Object {this} netId={netId} already has an owner. Use RemoveClientAuthority() first", this);
                return;
            }
        ...
        void Awake()
        {
            ...
            if (Application.isPlaying && sceneId != 0)
            {
                ...
                {
                    MirrorLog.LogError(name + "'s sceneId: " + sceneId.ToString("X") + " is already taken by: " + existing.name + ". Don't call Instantiate for NetworkIdentities that were in the scene since the beginning (aka scene objects). Otherwise the client won't know which object to use for a SpawnSceneObject message.");
                    Destroy(gameObject);
                }
                else
                {
        ...
        bool ThisIsASceneObjectWithPrefabParent(out GameObject prefab)
        {
            ...
            if (prefab == null)
            {
                MirrorLog.LogError("Failed to find prefab parent for scene object [name:" + gameObject.name + "]");
                return false;
            }
            return true;
        }
        ...
        void AssignSceneID()
        {
            ...
            if (sceneId == 0 || duplicate)
            {
                ...
                {
                    sceneId = randomId;
                    //MirrorLog.Log(name + " in scene=" + gameObject.scene.name + " sceneId assigned to: " + m_SceneId.ToString("X"));
                }
            }
        ...
        public void SetSceneIdSceneHashPartInternal()
        {
            ...
            if (LogFilter.Debug) MirrorLog.DebugLog(name + " in scene=" + gameObject.scene.name + " scene index hash(" + pathHash.ToString("X") + ") copied into sceneId: " + sceneId.ToString("X"));
        }
        ...
        void SetupIDs()
        {
            ...
            else if (PrefabStageUtility.GetCurrentPrefabStage() != null)
            {
                ...
                if (PrefabStageUtility.GetPrefabStage(gameObject) != null)
                {
                    // force 0 for prefabs
                    sceneId = 0;
                    //MirrorLog.Log(name + " @ scene: " + gameObject.scene.name + " sceneid reset to 0 because CurrentPrefabStage=" + PrefabStageUtility.GetCurrentPrefabStage() + " PrefabStage=" + PrefabStageUtility.GetPrefabStage(gameObject));
        ...
        internal void OnStartServer()
        {
            ...
            if (LogFilter.Debug) MirrorLog.DebugLog("OnStartServer " + this + " NetId:" + netId + " SceneId:" + sceneId);
            ...
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnStartServer:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal void OnStartClient()
        {
            ...
            if (LogFilter.Debug) MirrorLog.DebugLog("OnStartClient " + gameObject + " netId:" + netId);
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnStartClient:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal void OnStartLocalPlayer()
        {
            ...
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnStartLocalPlayer:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal void OnStartAuthority()
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnStartAuthority:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal void OnStopAuthority()
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnStopAuthority:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal void OnSetHostVisibility(bool visible)
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnSetLocalVisibility:" + e.Message + " " + e.StackTrace);
                }
            }
        }
        ...
        internal bool OnCheckObserver(NetworkConnection conn)
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnCheckObserver:" + e.Message + " " + e.StackTrace);
                }
            }
            return true;
        }
        ...
        internal void OnNetworkDestroy()
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    MirrorLog.LogError("Exception in OnNetworkDestroy:" + e.Message + " " + e.StackTrace);
                }
                isServer = false;
            }
        }
        ...
        bool OnSerializeSafely(NetworkBehaviour comp, NetworkWriter writer, bool initialState)
        {
            ...
            catch (Exception e)
            {
                ...
                MirrorLog.LogError("OnSerialize failed for: object=" + name + " component=" + comp.GetType() + " sceneId=" + sceneId.ToString("X") + "\n\n" + e);
            }
            ...
            if (LogFilter.Debug) MirrorLog.DebugLog("OnSerializeSafely written for object=" + comp.name + " component=" + comp.GetType() + " sceneId=" + sceneId.ToString("X") + "header@" + headerPosition + " content@" + contentPosition + " end@" + endPosition + " contentSize=" + (endPosition - contentPosition));
        ...
        internal void OnSerializeAllSafely(bool initialState, NetworkWriter ownerWriter, out int ownerWritten, NetworkWriter observersWriter, out int observersWritten)
        {
            ...
            {
                MirrorLog.LogError("Only 64 NetworkBehaviour components are allowed for NetworkIdentity: " + name + " because of the dirtyComponentMask");
                return;
            }
            ...
            foreach (NetworkBehaviour comp in NetworkBehaviours)
            {
                ...
                {
                    if (LogFilter.Debug) MirrorLog.DebugLog("OnSerializeAllSafely: " + name + " -> " + comp.GetType() + " initial=" + initialState);
        ...
        void OnDeserializeSafely(NetworkBehaviour comp, NetworkReader reader, bool initialState)
        {
            ...
            try
            {
                if (LogFilter.Debug) MirrorLog.DebugLog("OnDeserializeSafely: " + comp.name + " component=" + comp.GetType() + " sceneId=" + sceneId.ToString("X") + " length=" + contentSize);
                comp.OnDeserialize(reader, initialState);
            }
            catch (Exception e)
            {
                // show a detailed error and let the user know what went wrong
                MirrorLog.LogError("OnDeserialize failed for: object=" + name + " component=" + comp.GetType() + " sceneId=" + sceneId.ToString("X") + " length=" + contentSize + ". Possible Reasons:\n  * Do " + comp.GetType() + "'s OnSerialize and OnDeserialize calls write the same amount of data(" + contentSize + " bytes)? \n  * Was there an exception in " + comp.GetType() + "'s OnSerialize/OnDeserialize code?\n  * Are the server and client the exact same project?\n  * Maybe this OnDeserialize call was meant for another GameObject? The sceneIds can easily get out of sync if the Hierarchy was modified only in the client OR the server. Try rebuilding both.\n\n" + e);
            }
            ...
            if (reader.Position != chunkEnd)
            {
                ...
                MirrorLog.LogWarning("OnDeserialize was expected to read " + contentSize + " instead of " + bytesRead + " bytes for object:" + name + " component=" + comp.GetType() + " sceneId=" + sceneId.ToString("X") + ". Make sure that OnSerialize and OnDeserialize write/read the same amount of data in all cases.");
        ...
        void HandleRemoteCall(int componentIndex, int functionHash, MirrorInvokeType invokeType, NetworkReader reader)
        {
            if (gameObject == null)
            {
                MirrorLog.LogWarning(invokeType + " [" + functionHash + "] received for deleted object [netId=" + netId + "]");
                return;
            }
            ...
            if (0 <= componentIndex && componentIndex < NetworkBehaviours.Length)
            {
                ...
                {
                    MirrorLog.LogError("Found no receiver for incoming " + invokeType + " [" + functionHash + "] on " + gameObject + ",  the server and client should have the same NetworkBehaviour instances [netId=" + netId + "].");
                }
            }
            else
            {
                MirrorLog.LogWarning("Component [" + componentIndex + "] not found for [netId=" + netId + "]");
            }
        }
        ...
        internal void AddObserver(NetworkConnection conn)
        {
            if (observers == null)
            {
                MirrorLog.LogError("AddObserver for " + gameObject + " observer list is null");
                return;
            }
            ...
            if (LogFilter.Debug) MirrorLog.DebugLog("Added observer " + conn.address + " added for " + gameObject);
        ...
        public void RebuildObservers(bool initialize)
        {
            ...
            foreach (NetworkConnection conn in newObservers)
            {
                ...
                if (conn != null && conn.isReady)
                {
                    if (initialize || !observers.ContainsKey(conn.connectionId))
                    {
                        // new observer
                        conn.AddToVisList(this);
                        if (LogFilter.Debug) MirrorLog.DebugLog("New Observer for " + gameObject + " " + conn);
                        changed = true;
                    }
                }
            }
            ...
            foreach (NetworkConnection conn in observers.Values)
            {
                if (!newObservers.Contains(conn))
                {
                    ...
                    if (LogFilter.Debug) MirrorLog.DebugLog("Removed Observer for " + gameObject + " " + conn);
                    changed = true;
                }
            }
        ...
        public bool AssignClientAuthority(NetworkConnection conn)
        {
            if (!isServer)
            {
                MirrorLog.LogError("AssignClientAuthority can only be called on the server for spawned objects.");
                return false;
            }

            if (conn == null)
            {
                MirrorLog.LogError("AssignClientAuthority for " + gameObject + " owner cannot be null. Use RemoveClientAuthority() instead.");
                return false;
            }

            if (connectionToClient != null && conn != connectionToClient)
            {
                MirrorLog.LogError("AssignClientAuthority for " + gameObject + " already has an owner. Use RemoveClientAuthority() first.");
                return false;
            }
        ...
        public void RemoveClientAuthority()
        {
            if (!isServer)
            {
                MirrorLog.LogError("RemoveClientAuthority can only be called on the server for spawned objects.");
                return;
            }

            if (connectionToClient?.identity == this)
            {
                MirrorLog.LogError("RemoveClientAuthority cannot remove authority for a player object");
                return;
            }
