...
namespace Mirror
{
    ...
    {
        /// <summary>
        /// Singleton access for ease of use. Only works when using exactly one NetworkServer.
        /// </summary>
        ...
        /// </summary>
        public Dictionary<int, NetworkMessageDelegate> handlers = new Dictionary<int, NetworkMessageDelegate>();
        ...
        void Awake()
        {
            // make sure that there is only ever one NetworkServer (for now).
            // a couple of things don't allow multiple NetworkServers yet:
            // * Transport events call NetworkClient. what if we have multiple?
            if (singleton != null)
            {
                throw new Exception("Multiple NetworkServers detected. There can only be one (for now).");
            }
            // set singleton
            else
            {
        ...
        /// <returns></returns>
        public bool Listen(int maxConns)
        {
            Initialize();
        ...
        public bool SendToAll(int msgType, MessageBase msg, int channelId = Channels.DefaultReliable)
        {
        ...
        public bool SendToReady(NetworkIdentity identity, short msgType, MessageBase msg, int channelId = Channels.DefaultReliable)
        {
        ...
        // NetworkManager uses LateUpdate, so use LateUpdate here too.
        // Make sure to keep update order: NetworkManager->NetworkServer->NetworkClient
        // by sorting the components this way.
        /// <summary>
        ...
        public void SendToClient(int connectionId, int msgType, MessageBase msg)
        {
            if (connections.TryGetValue(connectionId, out NetworkConnectionToClient conn))
            {
        ...
        public void SendToClient<T>(int connectionId, T msg) where T : IMessageBase
        {
            if (connections.TryGetValue(connectionId, out NetworkConnectionToClient conn))
            {
                conn.Send(msg);
        ...
        public void SendToClientOfPlayer(NetworkIdentity identity, int msgType, MessageBase msg)
        {
            if (identity != null)
            {
        ...
        internal bool InternalReplacePlayerForConnection(NetworkConnection conn, GameObject player, bool keepAuthority)
        {
            NetworkIdentity identity = player.GetComponent<NetworkIdentity>();
            if (identity == null)
            {
        ...
        public bool SpawnWithClientAuthority(GameObject obj, NetworkConnection ownerConnection)
        {
        ...
        public bool SpawnWithClientAuthority(GameObject obj, Guid assetId, NetworkConnection ownerConnection)
        {
