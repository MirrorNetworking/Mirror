...
// some day, we can:
// -> rename NetworkServerComponent to NetworkServer (and remove the one)
...
namespace Mirror
{
    ...
    {
        // singleton for NetworkServer static class transition.
        // will be removed eventually.
        public static NetworkServerComponent singleton;

        bool initialized;
        public int maxConnections;

        /// <summary>Connection to host mode client (if any)</summary>
        public NetworkConnectionToClient localConnection { get; internal set; }
        ...
        public Dictionary<int, NetworkConnectionToClient> connections =
            new Dictionary<int, NetworkConnectionToClient>();

        /// <summary>Message Handlers dictionary, with mesageId as key</summary>
        internal Dictionary<ushort, NetworkMessageDelegate> handlers =
        ...
        public Action<NetworkConnection> OnConnectedEvent;
        public Action<NetworkConnection> OnDisconnectedEvent;
        ...
        internal void SetLocalConnection(LocalConnectionToClient conn)
        {
        ...
        public bool NoExternalConnections()
        {
        ...
        public void SendToAll<T>(T message, int channelId = Channels.Reliable, bool sendToReadyOnly = false)
        ...
        public void SendToReady<T>(T message, int channelId = Channels.Reliable)
            where T : struct, NetworkMessage
        {
            if (!active)
            {
        ...
        void SendToObservers<T>(NetworkIdentity identity, T message, int channelId = Channels.Reliable)
            where T : struct, NetworkMessage
        {
        ...
        public void SendToReady<T>(NetworkIdentity identity, T message, bool includeOwner = true, int channelId = Channels.Reliable)
            where T : struct, NetworkMessage =>
        ...
        // TODO put rpcs into NetworkServer.Update WorldState packet, then finally remove SendToReady!
        public void SendToReadyObservers<T>(NetworkIdentity identity, T message, int channelId)
            where T : struct, NetworkMessage
        {
        ...
        // Deprecated 2021-09-19
        [Obsolete("SendToReady(identity, message, ...) was renamed to SendToReadyObservers because that's what it does.")]
        public void SendToReady<T>(NetworkIdentity identity, T message, int channelId)
            where T : struct, NetworkMessage =>
        ...
        bool UnpackAndInvoke(NetworkConnectionToClient connection, NetworkReader reader, int channelId)
        {
        ...
        // called by transport
        internal void OnTransportData(int connectionId, ArraySegment<byte> data, int channelId)
        {
        ...
        internal void OnTransportDisconnected(int connectionId)
        {
        ...
        public void RegisterHandler<T>(Action<NetworkConnection, T, int> handler, bool requireAuthentication = true)
            where T : struct, NetworkMessage
        {
            ushort msgType = MessagePacking.GetId<T>();
            if (handlers.ContainsKey(msgType))
        ...
        public void ReplaceHandler<T>(Action<NetworkConnection, T> handler, bool requireAuthentication = true)
            where T : struct, NetworkMessage
        {
            ushort msgType = MessagePacking.GetId<T>();
            handlers[msgType] = MessagePacking.WrapHandler(handler, requireAuthentication);
        ...
        /// <summary>Replace a handler for message type T. Most should require authentication.</summary>
        public void ReplaceHandler<T>(Action<T> handler, bool requireAuthentication = true)
            where T : struct, NetworkMessage
        {
        ...
        public bool ReplacePlayerForConnection(NetworkConnection conn, GameObject player, bool keepAuthority = false)
        {
            NetworkIdentity identity = player.GetComponent<NetworkIdentity>();
            if (identity == null)
            {
        ...
        // This does NOT change the ready state of the connection, so it can
        // safely be used while changing scenes.
        public bool ReplacePlayerForConnection(NetworkConnection conn, GameObject player, Guid assetId, bool keepAuthority = false)
        {
            if (GetNetworkIdentity(player, out NetworkIdentity identity))
            {
                identity.assetId = assetId;
        ...
        void OnCommandMessage(NetworkConnection conn, CommandMessage msg, int channelId)
        {
            if (!conn.isReady)
            {
        ...
        // spawning ////////////////////////////////////////////////////////////
        ArraySegment<byte> CreateSpawnMessagePayload(bool isOwner, NetworkIdentity identity, PooledNetworkWriter ownerWriter, PooledNetworkWriter observersWriter)
        {
        ...
        internal void SendChangeOwnerMessage(NetworkIdentity identity, NetworkConnection conn)
        {
        ...
        void SpawnObject(GameObject obj, NetworkConnection ownerConnection)
        {
        ...
        public void UnSpawn(GameObject obj) => DestroyObject(obj, DestroyMode.Reset);
        ...
        void DestroyObject(NetworkIdentity identity, DestroyMode mode)
        {
        ...
        void DestroyObject(GameObject obj, DestroyMode mode)
        {
            if (obj == null)
            {
        ...
        public void Destroy(GameObject obj) => DestroyObject(obj, DestroyMode.Destroy);
        ...
        internal void AddAllReadyServerConnectionsToObservers(NetworkIdentity identity)
        {
        ...
        // internal for tests
        internal readonly HashSet<NetworkConnection> newObservers = new HashSet<NetworkConnection>();

        // rebuild observers default method (no AOI) - adds all connections
        void RebuildObserversDefault(NetworkIdentity identity, bool initialize)
        {
        ...
        void RebuildObserversCustom(NetworkIdentity identity, bool initialize)
        {
        ...
        public void RebuildObservers(NetworkIdentity identity, bool initialize)
        {
        ...
        NetworkWriter GetEntitySerializationForConnection(NetworkIdentity identity, NetworkConnectionToClient connection)
        {
        ...
        void BroadcastToConnection(NetworkConnectionToClient connection)
        {
        ...
        // internal for tests
        internal readonly List<NetworkConnectionToClient> connectionsCopy =
        ...
        // (we add this to the UnityEngine in NetworkLoop)
        internal void NetworkEarlyUpdate()
        {
