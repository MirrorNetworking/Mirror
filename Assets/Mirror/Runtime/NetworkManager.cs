...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        //
        internal bool StartServerInternal(bool automaticallySpawnObjects)
        {
            InitializeSingleton();
            ...
            {
                // spawn objects, but only if not starting server as host mode.
                // spawning objects calls OnStartServer
                ...
                //           }
                //
                //    => so we need to call OnStartServer AFTER the host
                //       connected.
                if (automaticallySpawnObjects)
        ...
        public virtual void StartHost()
        {
            ...
            // start server, but don't spawn objects yet. wait until the host
            // connected successfully.
            if (StartServerInternal(false))
            {
                // now connect the host. this happens immediately.
                NetworkClient.SetupLocalConnection();
                ConnectLocalClient();
                OnStartClient();

                // finish the StartServer process by spawning objects. this will
                // call OnStartServer for all spawned objects.
                NetworkServer.SpawnObjects();
            }
        }
