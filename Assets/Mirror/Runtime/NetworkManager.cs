...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        {
            // spawn server objects. afterwards the host server is fully set up.
            NetworkServer.SpawnObjects();

            // start host client AFTER server was fully set up.
            // note that there is a bug where SyncVar hooks aren't called for
            // the host client if we set SyncVars in OnStartServer. that is
            // because the host client isn't connected yet.
            // https://github.com/vis2k/Mirror/issues/1397
            // => the solution is NOT to connect the host client before
            //    spawning, because then OnStartClient would happen before the
            //    host server was fully started (= unwanted situations), and
            //    most importantly, StartClient doesn't finish immediately if
            //    host connections go over the transport. so we would encounter
            //    a race conditions.
            // => see the issue for better solutions.
            // NOTE: this 'bug' was in UNET since day one. we briefly had a
            //       workaround in mirror for about a week before moving to
            //       local connections that go over transport.
            Debug.Log("NetworkManager.StartHost: connecting host client...");
