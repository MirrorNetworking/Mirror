...
namespace Mirror.Tests
{
    ...
    public class WeaverTest
    {
        ...
        {
            string networkServerGetActive = Weaver.Weaver.NetworkBehaviourIsServer.ToString();
            CheckAddedCode(networkServerGetActive, "ServerOnlyMethod");
        }

        static void CheckAddedCodeClient()
        {
            string networkClientGetActive = Weaver.Weaver.NetworkBehaviourIsClient.ToString();
        ...
        static void CheckAddedCode(string addedString, string methodName)
        {
            ...
            using (var assembly = AssemblyDefinition.ReadAssembly(assemblyName))
            {
                ...
                Instruction ldarg = body.Instructions[0];
                Assert.AreEqual(ldarg.OpCode, OpCodes.Ldarg_0);
