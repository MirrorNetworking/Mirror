...
public class NTRCustomSendInterval : NetworkTransformBase
{
    ...
    protected int lastClientCount = 0;
    ...
    // We are checking time locally here, marking the first send when OnSerialize is called
    // for the first time. Will this time interval check deviate from the Network LateUpdate
    // check?
...
#endif
        {
            if (sendIntervalCounter == sendIntervalMultiplier) sendIntervalCounter = 0;
            sendIntervalCounter++;
        }
    }
