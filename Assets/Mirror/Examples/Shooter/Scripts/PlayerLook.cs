    ...
    #pragma warning restore CS0109 // member does not hide accessible member
        ...
        void LateUpdate()
        {
            ...
            if (!Utils.IsCursorOverUserInterface())
            {
                ...
                if (distance == 0)
                {
                    player.playerWeapon.SetFirstPerson();
                }
                else
                {
                    player.playerWeapon.SetThirdPerson();
                }
            }
        ...
        IEnumerator DelayMouseLock()
        {
            // force wait before locking mouse, so we can press UI, after unlocking with ESC
            // needed to be more than 5 frames during testing, but framerate/vsync and other factors effect it
            // so hard to know exactly how long?
