...
namespace Mirror.Examples.NetworkRoom
{
    public class PlayerScore : NetworkBehaviour
    {
        ...
        void OnControllerColliderHit(ControllerColliderHit hit)
        {
            ...
            {
                if (LogFilter.Debug) MirrorLog.DebugLog("OnControllerColliderHit {0}[{1}] with {2}[{3}]", name, netId, controllerColliderHitObject.name, controllerColliderHitObject.GetComponent<NetworkIdentity>().netId);
