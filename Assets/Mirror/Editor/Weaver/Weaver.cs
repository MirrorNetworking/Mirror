...
namespace Mirror.Weaver
{
    ...
    class Weaver
    {
        ...
        {
            //ExportedTypes has 0 items
            TypeDefinition type = CorLibModule.GetType(fullName) ?? CorLibModule.ExportedTypes.FirstOrDefault(t => t.FullName == fullName)?.Resolve() ?? CorLibModule.Types.FirstOrDefault(t => t.FullName == fullName)?.Resolve();
            if (type != null)
            {
