...
public class HexSpatialHash3DInterestManagement : InterestManagement
{
    ...
    [Range(10, 5000), Tooltip("Cell height effects all 3 layers")]
    ...
    readonly Dictionary<NetworkIdentity, (Cell cell, Vector3 worldPos)> lastIdentityPositions = new Dictionary<NetworkIdentity, (Cell, Vector3)>();

    // Tracks the last known cell position and world position of each player's connection (observer)
    readonly Dictionary<NetworkConnectionToClient, (Cell cell, Vector3 worldPos)> lastConnectionPositions = new Dictionary<NetworkConnectionToClient, (Cell, Vector3)>();

    // Pre-allocated array for storing neighbor cells (center + 6 neighbors per layer x 3 layers)
    readonly Cell[] neighborCells = new Cell[21];
        ...
        if (NetworkTime.time - lastRebuildTime >= rebuildInterval)
        {
            ...
            foreach (NetworkConnectionToClient conn in NetworkServer.connections.Values)
                if (conn?.identity != null) // Ensure connection and its identity exist
                {
                    ...
                    if (!lastConnectionPositions.TryGetValue(conn, out (Cell cell, Vector3 worldPos) last) ||
                        Vector3.Distance(position, last.worldPos) >= minMoveDistance)
                    {
                        Cell cell = grid.WorldToCell(position); // Convert world position to grid cell
            ...
            foreach (NetworkConnectionToClient conn in NetworkServer.connections.Values)
            {
                if (conn?.identity == null || !lastConnectionPositions.TryGetValue(conn, out (Cell cell, Vector3 worldPos) connPos))
                    continue;
    ...
    void UpdateIdentityPosition(NetworkIdentity identity)
    {
        ...
        if (lastIdentityPositions.TryGetValue(identity, out (Cell cell, Vector3 worldPos) previous))
        {
    ...
    void AddToCell(Cell cell, NetworkIdentity identity)
    {
    ...
    public override bool OnCheckObserver(NetworkIdentity identity, NetworkConnectionToClient newObserver)
    {
        ...
        if (!lastIdentityPositions.TryGetValue(identity, out (Cell cell, Vector3 worldPos) identityPos) ||
            !lastConnectionPositions.TryGetValue(newObserver, out (Cell cell, Vector3 worldPos) observerPos))
    ...
    public override void OnDestroyed(NetworkIdentity identity)
    {
        ...
        if (lastIdentityPositions.TryGetValue(identity, out (Cell cell, Vector3 worldPos) pos))
        {
    ...
    void OnDrawGizmos()
    {
        ...
        if (NetworkClient.localPlayer != null)
        {
            ...
            Cell playerCell = grid.WorldToCell(playerPosition);
