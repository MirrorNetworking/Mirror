...
internal class HexGrid3D
{
    ...
    internal HexGrid3D(ushort visRange, ushort height)
    {
        ...
        // Cell height is absolute...don't double it
        cellHeight = height;

        // Offset to center the grid at world origin
        // Cell height must be divided by 2 for vertical centering
    ...
    // Precomputed array of neighbor offsets as Cell structs (center + 6 per layer x 3 layers)
    static readonly Cell[] neighborCellsBase = new Cell[]
    {
        // Center
        new Cell(0, 0, 0),
        // Upper layer (1) and its 6 neighbors
        new Cell(0, 0, 1),
        new Cell(1, -1, 1), new Cell(1, 0, 1), new Cell(0, 1, 1),
        new Cell(-1, 1, 1), new Cell(-1, 0, 1), new Cell(0, -1, 1),
        // Same layer (0) - 6 neighbors
        new Cell(1, -1, 0), new Cell(1, 0, 0), new Cell(0, 1, 0),
        new Cell(-1, 1, 0), new Cell(-1, 0, 0), new Cell(0, -1, 0),
    ...
    internal Vector3 CellToWorld(Cell cell)
    {
    ...
    internal Cell WorldToCell(Vector3 position)
    {
    ...
    Cell RoundToCell(float q, float r, int layer)
    {
        ...
        return new Cell(qInt, rInt, layer);
    }

    // Populates the provided array with neighboring cells around a given center cell
    internal void GetNeighborCells(Cell center, Cell[] neighbors)
    {
...
internal struct Cell
{
    ...
    internal Cell(int q, int r, int layer)
    {
        this.q = q;
        this.r = r;
        this.layer = layer;
