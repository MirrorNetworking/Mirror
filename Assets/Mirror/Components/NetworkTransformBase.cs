...
namespace Mirror
{
    public abstract class NetworkTransformBase : NetworkBehaviour
    {
        ...
        [Tooltip("Synch Global position and rotation")]
        ...
        public class DataPoint
        {
            ...
            public Vector3 thePosition; //could be global or local dep on bool
        ...
        static float EstimateMovementSpeed(DataPoint from, DataPoint to, Transform transform, float sendInterval, bool useGlobalPosition)
        {
            Vector3 delta;
            if (useGlobalPosition)
            {
                delta = to.thePosition - (from != null ? from.thePosition : transform.position);
            } else
            {
                delta = to.thePosition - (from != null ? from.thePosition : transform.localPosition);
            }
        ...
        void DeserializeFromReader(NetworkReader reader)
        {
            ...
            DataPoint temp = new DataPoint
            {
                ...
                thePosition = reader.ReadVector3()
            };

            // deserialize rotation & scale
            temp.theRotation = reader.ReadQuaternion();
            temp.theScale = reader.ReadVector3();

            temp.timeStamp = Time.time;

            // movement speed: based on how far it moved since last time
            // has to be calculated before 'start' is overwritten
            temp.movementSpeed = EstimateMovementSpeed(goal, temp, targetComponent.transform, syncInterval, useGlobalPosition);
            ...
            if (start == null)
            {
                if (useGlobalPosition)
                {
                    start = new DataPoint
                    {
                        ...
                        thePosition = targetComponent.transform.position,
                        theRotation = targetComponent.transform.rotation,
                        theScale = targetComponent.transform.localScale,
                ...
                } else
                {
                    start = new DataPoint
                    {
                        ...
                        thePosition = targetComponent.transform.localPosition,
                        theRotation = targetComponent.transform.localRotation,
                        theScale = targetComponent.transform.localScale,
                        movementSpeed = temp.movementSpeed
                    };
                }
            ...
            {
                float oldDistance = Vector3.Distance(start.thePosition, goal.thePosition);
                float newDistance = Vector3.Distance(goal.thePosition, temp.thePosition);
                ...
                if (useGlobalPosition)
                {
                    if (Vector3.Distance(targetComponent.transform.position, start.thePosition) < oldDistance + newDistance)
                    {
                        start.thePosition = targetComponent.transform.position;
                        start.theRotation = targetComponent.transform.rotation;
                        start.theScale = targetComponent.transform.localScale;
                    }
                } else
                {
                    if (Vector3.Distance(targetComponent.transform.localPosition, start.thePosition) < oldDistance + newDistance)
                    {
                        start.thePosition = targetComponent.transform.localPosition;
                        start.theRotation = targetComponent.transform.localRotation;
                        start.theScale = targetComponent.transform.localScale;
                    }
                }
            }
        ...
        void CmdClientToServerSync(byte[] payload)
        {
            ...
            if (isServer && !isClient) ApplyPositionRotationScale(goal.thePosition, goal.theRotation, goal.theScale);
        ...
        static Vector3 InterpolatePosition(DataPoint start, DataPoint goal, Vector3 currentPosition)
        {
            if (start != null)
            {
                ...
                return Vector3.MoveTowards(currentPosition, goal.thePosition, speed * Time.deltaTime);
            }
        ...
        static Quaternion InterpolateRotation(DataPoint start, DataPoint goal, Quaternion defaultRotation)
        {
            if (start != null)
            {
                ...
                return Quaternion.Slerp(start.theRotation, goal.theRotation, t);
            }
        ...
        static Vector3 InterpolateScale(DataPoint start, DataPoint goal, Vector3 currentScale)
        {
            ...
            {
                float t = CurrentInterpolationFactor(start, goal);
                return Vector3.Lerp(start.theScale, goal.theScale, t);
            }
            return currentScale;
        }
        ...
        bool HasEitherMovedRotatedScaled()
        {
            ...
            if (useGlobalPosition)
            {
                bool moved = Vector3.Distance(lastPosition, targetComponent.transform.position) > localPositionSensitivity;
                bool scaled = Vector3.Distance(lastScale, targetComponent.transform.localScale) > localScaleSensitivity;
                bool rotated = Quaternion.Angle(lastRotation, targetComponent.transform.rotation) > localRotationSensitivity;

                // save last for next frame to compare
                // (only if change was detected. otherwise slow moving objects might
                //  never sync because of C#'s float comparison tolerance. see also:
                //  https://github.com/vis2k/Mirror/pull/428)
                change = moved || rotated || scaled;
                if (change)
                {
                    // local position/rotation for VR support
        ...
        static void DrawDataPointGizmo(DataPoint data, Color color)
        {
            ...
            // draw position
            Gizmos.color = color;
            Gizmos.DrawSphere(data.thePosition + offset, 0.5f);

            // draw forward and up
            // like unity move tool
            Gizmos.color = Color.blue;
            Gizmos.DrawRay(data.thePosition + offset, data.theRotation * Vector3.forward);

            // like unity move tool
            Gizmos.color = Color.green;
            Gizmos.DrawRay(data.thePosition + offset, data.theRotation * Vector3.up);
        }

        static void DrawLineBetweenDataPoints(DataPoint data1, DataPoint data2, Color color)
        {
            Gizmos.color = color;
            Gizmos.DrawLine(data1.thePosition, data2.thePosition);
        }
