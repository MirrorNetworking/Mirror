...
namespace Mirror
{
    [AddComponentMenu("NetworkTransform Hybrid V2")]
    public class NetworkTransformHybrid2 : NetworkBehaviour
    {
        ...
        [SerializeField] protected SyncSettings syncSettings = SyncSettings.Default;
        ...
        [SerializeField]
        public InterpolateSettings interpolateSettings = InterpolateSettings.Default;
        ...
        [Header("Send Rates")]
        [Tooltip("Number of full snapshots to send per second (Hz).")]
        [Range(1, 120)]
        public byte fullSendRate = 1;
        float fullSendInterval => 1 / fullSendRate;
        double lastFullSendIntervalTime = double.MinValue;
        byte lastSentFullSyncIndex = 0;
        SyncDataFull lastSentFullSyncData;
        QuantizedSnapshot lastSentFullQuantized;
        SyncDataFull lastReceivedFullSyncData;
        QuantizedSnapshot lastReceivedFullQuantized;

        [Tooltip("Number of delta snapshots to send per second (Hz).")]
        [Range(1, 120)]
        public byte deltaSendRate = 30;
        float deltaSendInterval => 1 / deltaSendRate;
        ...
        protected double timeStampAdjustment => Math.Max(0, deltaSendInterval - NetworkServer.sendInterval);
        ...
        protected override void OnValidate()
        {
            ...
            if ((fullHeader & (FullHeader.CompressRot & FullHeader.UseEulerAngles)) > 0)
                fullHeader &= ~FullHeader.CompressRot;

            deltaSendRate = (byte)Math.Max(deltaSendRate, fullSendRate);
            fullSendRate = (byte)Math.Min(deltaSendRate, fullSendRate);

            // Unity doesn't support setting world scale.
            // Force disable syncScale in world mode.
            if (coordinateSpace == ReferenceSpace.World)
                syncSettings &= ~SyncSettings.SyncScale;
        }
        ...
        {
            if (!NetworkClient.spawned.TryGetValue(msg.netId, out NetworkIdentity networkIdentity)) return;

            ((NetworkTransformHybrid2)networkIdentity.NetworkBehaviours[msg.componentId]).OnServerToClientSyncFull(msg.syncData);
        }
        ...
        protected static void ServerToClientSyncDeltaHandler(SyncDataDeltaMsg msg)
        {
            ...
            ((NetworkTransformHybrid2)networkIdentity.NetworkBehaviours[msg.componentId]).OnServerToClientSyncDelta(msg.syncData);
        }
        ...
        {
            if (!NetworkServer.spawned.TryGetValue(msg.netId, out NetworkIdentity networkIdentity)) return;
            ...
            NetworkTransformHybrid2 nT = (NetworkTransformHybrid2)networkIdentity.NetworkBehaviours[msg.componentId];
        ...
        {
            // lastSentFullSyncIndex is a byte that will wrap around.
            // It's intended that the incremented lastSentFullSyncIndex be used
            // if updateIndex is true, not increment after the constructor call.
            return new SyncDataFull(
                updateIndex ? ++lastSentFullSyncIndex : lastSentFullSyncIndex,
                fullHeader,
                GetPosition(),
                GetRotation(),
                GetScale()
