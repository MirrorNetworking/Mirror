...
namespace Mirror
{
    ...
    public class NetworkProximityChecker : NetworkBehaviour
    {
        ...
        private void Add3DHits(HashSet<NetworkConnection> observers)
        {
            ...
            if (hitCount == hitsBuffer3D.Length) MirrorLog.LogWarning("NetworkProximityChecker's OverlapSphere test for " + name + " has filled the whole buffer(" + hitsBuffer3D.Length + "). Some results might have been omitted. Consider increasing buffer size.");
        ...
        private void Add2DHits(HashSet<NetworkConnection> observers)
        {
            ...
            if (hitCount == hitsBuffer2D.Length) MirrorLog.LogWarning("NetworkProximityChecker's OverlapCircle test for " + name + " has filled the whole buffer(" + hitsBuffer2D.Length + "). Some results might have been omitted. Consider increasing buffer size.");
