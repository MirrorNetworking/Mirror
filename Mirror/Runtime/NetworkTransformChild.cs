...
namespace Mirror
{
    ...
    public class NetworkTransformChild : NetworkBehaviour
    {
        ...
        void FixedUpdateClient()
        {
            ...
            if (m_LastClientSyncTime != 0)
            {
                m_Target.localPosition = m_InterpolateMovement > 0 ? Vector3.Lerp(m_Target.localPosition, m_TargetSyncPosition, m_InterpolateMovement) : m_TargetSyncPosition;

                m_Target.localRotation = m_InterpolateRotation > 0 ? Quaternion.Slerp(m_Target.localRotation, m_TargetSyncRotation3D, m_InterpolateRotation) : m_TargetSyncRotation3D;
            }
        }
