    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        internal void AddObserver(NetworkConnection conn)
        {
            ...
            if (m_ObserverConnections.Contains(conn.connectionId))
            {
                ...
                if (LogFilter.logWarn) { Debug.LogWarning("AddObserver: connectionId=" + conn.connectionId + " already added for " + name + ". Maybe this function was called twice. Or maybe the transport layer generated the same connectionId twice, which would be a huge problem."); }
                return;
            }
