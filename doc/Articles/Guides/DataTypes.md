...
MirrorNG supports a number of data types you can use with these, including:
...
When the sync data arrives at the client, the referenced game object may not yet exist on that client, resulting in null values in the sync data.  This is because interally MirrorNG passes the `netId` from the `NetworkIdentity` and tries to look it up on the client's `NetworkIdentity.spawned` dictionary.
...
Sometimes you may want to serialize data which uses a different type not supported by MirrorNG, such as DateTime or System.Uri
...
Sometimes you might want to send a polymorphic data type to your commands. MirrorNG does not serialize the type name to keep messages small and for security reasons, therefore MirrorNG cannot figure out the type of object it received by looking at the message.
...
class Player : NetworkBehaviour
{
    ...
    {
        // IMPORTANT: this does not work. MirrorNG will pass you an object of type item
...
People often want to send scriptable objects from the client or server. For example, you may have a bunch of swords created as scriptable objects and you want put the equipped sword in a syncvar. This will work fine, MirrorNG will generate a reader and writer for scriptable objects by calling ScriptableObject.CreateInstance and copy all the data. 
